...
def load_and_prepare_data():
    ...
    y = pd.read_csv(os.path.join(PROCESSED_DIR, 'y_train.csv'))['Crop_encoded']
    ...
    label_mapping_df.to_csv(os.path.join(PROCESSED_DIR, 'label_mapping.csv'), index=False)
    print(f"Label mapping saved to {os.path.join(PROCESSED_DIR, 'label_mapping.csv')}")
...
def plot_results(results_df):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(PROCESSED_DIR, 'comprehensive_model_comparison.png'), dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    # === Hybrid Models ===
    print(f"\n{'='*70}")
    print("🤝 BUILDING HYBRID ENSEMBLE MODELS (TOP 3)")
    print(f"{'='*70}")
    top3 = results_df.head(3)['model_name'].tolist()
    print(f"Top 3 models: {top3}")
    estimators = [(name, trained_models[name]) for name in top3]

    # Voting Classifier (Hard)
    voting_hard = VotingClassifier(estimators=estimators, voting='hard')
    results_vh, model_vh = evaluate_model(
        voting_hard, 'Voting (Hard)', X_train, X_test, y_train, y_test, scaler
    )
    all_results.append(results_vh)
    trained_models['Voting (Hard)'] = model_vh

    # Voting Classifier (Soft) - only if all have predict_proba
    if all(hasattr(trained_models[name], 'predict_proba') for name in top3):
        voting_soft = VotingClassifier(estimators=estimators, voting='soft')
        results_vs, model_vs = evaluate_model(
            voting_soft, 'Voting (Soft)', X_train, X_test, y_train, y_test, scaler
        )
        all_results.append(results_vs)
        trained_models['Voting (Soft)'] = model_vs
    ...
    results_stack, model_stack = evaluate_model(
        stacking, 'Stacking', X_train, X_test, y_train, y_test, scaler
    )
    all_results.append(results_stack)
    trained_models['Stacking'] = model_stack
    ...
    # Save results
    results_df.to_csv(os.path.join(PROCESSED_DIR, 'model_comparison_results.csv'), index=False)

    # Save the best model
    best_model_name = results_df.iloc[0]['model_name']
    best_model = trained_models[best_model_name]
    joblib.dump(best_model, os.path.join(PROCESSED_DIR, f'best_model_{best_model_name.lower().replace(" ", "_")}.pkl'))
    
    print(f"\n🎯 Best Model: {best_model_name} with accuracy: {results_df.iloc[0]['accuracy']:.4f}")
    print(f"📊 Results saved to: {os.path.join(PROCESSED_DIR, 'model_comparison_results.csv')}")
    print(f"💾 Best model saved to: {os.path.join(PROCESSED_DIR, f'best_model_{best_model_name.lower().replace(' ', '_')}.pkl')}")
    
    # Create visualizations
    try:
        plot_results(results_df)
        print(f"📈 Visualizations saved to: {os.path.join(PROCESSED_DIR, 'comprehensive_model_comparison.png')}")
    except Exception as e:
